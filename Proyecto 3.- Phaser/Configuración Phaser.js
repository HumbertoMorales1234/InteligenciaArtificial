...
function create() {
    ...
    bala2 = juego.add.sprite(w-100, h, 'bala')
    ...
    avanza = juego.input.keyboard.addKey(Phaser.Keyboard.A);
    salto = juego.input.keyboard.addKey(Phaser.Keyboard.SPACEBAR);


    
    nnNetwork =  new synaptic.Architect.Perceptron(4, 15, 15, 2);
...
function update() {
    ...
    if(modoAuto == false){
        ...
        if( avanza.isDown && jugador.position.x <= posicionOriginal){
            avanzar();
        }
    }
    ...
    if( modoAuto ==false ){
        ...
        datosEntrenamiento.push({
                ...
                'output':  [ estatuSuelo, estatusAvanza ] 
        });

        console.log(despBala + " " +velocidadBala + " "+  despBala2+" " + velocidadBala2+" "+ estatuSuelo+" "+estatusAvanza);
   }
